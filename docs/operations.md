...
`binary-move-tablebase-scope` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies tablebase behavior through layout fixtures, with stable geometry snapshots and local-only command execution.
